...
ConVar Sound;
ConVar Soundvolume;
ConVar TeammateSound;

int bSound = 0;
float bSoundvolume = 0.00;
bool g_bLateLoaded = false;
bool bTeammateSound = false;
...
public void OnPluginStart()
{
	...
	TeammateSound        =    CreateConVar( "hs_teammate_enable",     "0",    "Also Play HeadShot Sound To Same Teamate? || 1= Yes || 0= No", _, true, 0.0, true, 2.0);
	...
	HookConVarChange(TeammateSound, OnSettingsChanged);
...
public void OnConfigsExecuted()
{
	...
	bTeammateSound = GetConVarBool(TeammateSound);
...
public int OnSettingsChanged(Handle convar, const char[] oldValue, const char[] newValue)
{
	...
	if(convar == TeammateSound)
	{
		bTeammateSound = TeammateSound.BoolValue;
	}
...
public Action Event_PlayerDeath_Pre(Handle event, const char[] name, bool dontBroadcast)
{
	...
	}else if(!bTeammateSound)
	{
		if (IsValidClient(attacker) && IsValidClient(victim))
		{
			if(GetClientTeam(attacker) != GetClientTeam(victim) && headshot)
			{
				EmitSoundToClient(attacker, iFileSound, _, _, _, _, bSoundvolume);
			}
		}
...
public Action OneHitDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	...
	}else if(!bTeammateSound)
	{
		if (IsValidClient(attacker) && IsValidClient(victim))
		{
			if(GetClientTeam(attacker) != GetClientTeam(victim) && (damagetype &= CS_DMG_HEADSHOT))
			{
				EmitSoundToClient(attacker, iFileSound, _, _, _, _, bSoundvolume);
			}
		}
